...
def evaluate(description):
    ...
    for i_dom, domain_name in enumerate(domain_seq_loop):
        if i_dom == 0 or "reset_each_shift" in cfg.SETTING:
            try:
                if i_dom == 0:
                    model.model_states, model.optimizer_state = model.copy_model_and_optimizer()
                model.reset()
        ...
        for severity in severities:
            ...
            if i_dom == 0 and cfg.CORRUPTION.DATASET!="ccc":
            ...
            # evaluate the model
            # acc, domain_dict, num_samples, accs, correct_confidences, wrong_confidences = get_accuracy(
            acc, domain_dict, num_samples, accs = get_accuracy(
                model,
                ...
                device=device,
                accs=accs,
                oracle=True if cfg.MODEL.ADAPTATION=="tent_oracle" else False,
