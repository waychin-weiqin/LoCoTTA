...
def get_accuracy(model: torch.nn.Module,
                 ...
                 accs: list,
                 oracle: bool,
                 correct_confidences: list = None,
                 wrong_confidences: list = None):
    ...
    with torch.no_grad():
        for i, data in enumerate(data_loader):
            ...
            if oracle:
                imgs = [img.to(device) for img in imgs] if isinstance(imgs, list) else imgs.to(device)
                output = model([imgs, labels.to(device)])
            else:
            ...
            confidences = F.softmax(output, dim=1).max(1)[0]
            correct_mask = predictions == labels.to(device)
            # correct_confidences.append(confidences[correct_mask].mean().item())
            # wrong_confidences.append(confidences[~correct_mask].mean().item())
            correct = (correct_mask).float().sum()
            num_correct += correct

            # model.errs.append(correct.item() / predictions.size(0))
            ...
            accs.append(correct.item() / imgs.shape[0])

    accuracy = num_correct.item() / num_samples
    return accuracy, domain_dict, num_samples, accs#, correct_confidences, wrong_confidences
