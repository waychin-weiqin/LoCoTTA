...
def block_selection(model, src_loader, blocks, num_classes, alpha = 0.75):
    ...
    for i in range(len(diff_list)):
        print("[INFO]", blocks[i], diff_list[i])
...
class DPLOT(TTAMethod):
    ...
    def __init__(self, cfg, model, num_classes):
        ...
        _, self.src_loader = get_source_loader(dataset_name="cifar100",
                                               data_root_dir="/home/wei/Documents/Dataset/cifar/CIFAR-100/", adaptation=cfg.MODEL.ADAPTATION,
                                               batch_size=batch_size_src, ckpt_path=cfg.MODEL.CKPT_PATH,
                                               percentage=cfg.SOURCE.PERCENTAGE,
                                               workers=min(cfg.SOURCE.NUM_WORKERS, os.cpu_count()),
                                               use_clip=False, preprocess=transforms.Compose([transforms.ToTensor()]),)
        ...
        # ImageNet-C (ResNet50)
        # blocks = ['layer1.0', 'layer1.1', 'layer1.2', 'layer2.1']
    ...
    def forward_and_adapt(self, x):
        ...
        ### Recompute Outputs for Second Stage
        # Assuming outputs and outputs_ema_avg are recomputed
        outputs = self.model(imgs)  # Recompute the outputs or use detach()
        outputs_ema_avg = self.model_ema(imgs)  # Similarly recompute if needed

        ### Second Stage
        celoss = symmetric_cross_entropy(outputs[:len(imgs_test)], outputs_ema_avg[:len(imgs_test)],).mean() + symmetric_cross_entropy(outputs[len(imgs_test):], outputs_ema_avg[:len(imgs_test)]).mean()
        celoss.backward()
