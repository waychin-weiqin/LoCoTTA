...
class EATA(TTAMethod):
    ...
    def __init__(self, cfg, model, num_classes):
        ...
        self.track_ids_1 = []  # track the ids of samples after first filtering
        self.track_ids_2 = []  # track the ids of samples after second filtering
        self.track_ids_final = []  # track the ids of samples after final filtering
        self.track_entropy = []  # track the entropy of ema after first filtering
        self.output_confidence = []  # track the confidence of the model output
    ...
    def loss_calculation(self, x):
        ...
        self.track_ids_1.append(outputs.size(0) - filter_ids_1[0].size(0))
        ...
        if self.current_model_probs is not None:
            ...
            self.track_ids_2.append(len(filter_ids_1[0]) - len(filter_ids_2[0]))
            entropys = entropys[filter_ids_2]
            self.track_entropy.append(torch.sum(self.current_model_probs.log() * self.current_model_probs).item())
        ...
        if self.current_model_probs is not None:
            if len(filter_ids_2[0]) != entropys.size(0):
                print("[ERROR] Size mismatched: filter_ids_2[0] != entropys.size(0)")

            # self.track_ids_final.append(len(filter_ids_2[0]))
        ...
        if self.fishers is not None:
            ...
            for name, param in self.model.named_parameters():
                if name in self.fishers:
                    ...
                    ewc_loss += self.fisher_alpha * ((param - self.fishers[name][1]) ** 2).sum() 
            loss += ewc_loss
    ...
    def forward_and_adapt(self, x):
        if self.mixed_precision and self.device == "cuda":
            ...
            if perform_update:
                ...
                self.optimizer.step()
            self.optimizer.zero_grad()
            self.output_confidence.append(outputs.softmax(1).max(1)[0].mean().item())
        return outputs
